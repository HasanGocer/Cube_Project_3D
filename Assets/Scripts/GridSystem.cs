...
public class GridSystem : MonoSingleton<GridSystem>
{
    ...
    public float scale;
    [SerializeField] private GameObject _cubeTemplatePosition, _cubeParent, _finishBox;
    [SerializeField] private float _positionDistance;
    [SerializeField] private int _OPCubeCount;
    public float particalWaitTime;
    ...
    public void AllCubeSetActiveFalse(int lineCount, int columnCount, GameObject[,] GridGameObject)
    {
        for (int i1 = 0; i1 < lineCount; i1++)
        {
            for (int i2 = 0; i2 < columnCount; i2++)
            {
                if (GridGameObject[i1, i2] != null)
                {
                    CrushCube crushCube = CrushCube.Instance;
                    StartCoroutine(crushCube.CallPartical(GridGameObject[i1, i2], GridGameObject[i1, i2].GetComponent<CubeID>(), crushCube.OPCubeCrashCount, particalWaitTime));
                    CubeAddObjectPool(GridGameObject[i1, i2]);
                }
            }
        }
    }
    ...
    private void GameObjectMatrisPlacement(GameObject cubeTemplatePosition, GameObject cubeParent, int lineCount, int columnCount, int OPCubeCount, GameObject[,] GridGameObject, int[,] GridInt)
    {
        ...
        for (int countForLine = 0; countForLine < lineCount; countForLine++)
        {
            for (int countForColumn = 0; countForColumn < columnCount; countForColumn++)
            {
                ...
                CubeFixedFunc(obj, GridInt[countForLine, countForColumn], countForLine, countForColumn);
            }
        }
    }
    ...
    {
        return scale = ((5 * positionDistance) + 1) / (7 * lineCount);
    }
    ...
    {
        obj.transform.position = new Vector3(cubeTemplatePosition.transform.position.x + ((scale / 2) - (positionDistance / 2) + ((scale / 5) * 7 * countForLine)), cubeTemplatePosition.transform.position.y, cubeTemplatePosition.transform.position.z + ((scale / 5) * 6 * countForColumn));
    ...
    private void CubeFixedFunc(GameObject obj, int cubeCount, int lineCount, int columnCount)
    {
        CubeID cubeID = obj.GetComponent<CubeID>();
        cubeID.cubeCount = cubeCount;
        cubeID.LineCount = lineCount;
        cubeID.columnCount = columnCount;
    }
}